...
@section('title', __('site.auth.passwords.reset.title').' | '.__('site.meta.brand'))
    ...
    <section class="px-6 pt-12 lg:px-8 lg:pt-16">
        <div class="mx-auto max-w-3xl">
            <div class="surface-panel rounded-[2.2rem] p-8 sm:p-10">
                ...
                <h1 class="mt-6 text-4xl font-semibold text-white sm:text-5xl">{{ __('site.auth.passwords.reset.title') }}</h1>
                <p class="mt-5 max-w-2xl text-base leading-8 text-slate-300">{{ __('site.auth.passwords.reset.copy') }}</p>
                ...
                <form method="POST" action="{{ route('password.update') }}" class="mt-8 space-y-5">
                    ...
                    <label class="block">
                        <span class="mb-2 block text-sm font-medium text-slate-200">{{ __('site.auth.passwords.reset.password') }}</span>
                        <input
                            type="password"
                            name="password"
                            class="w-full rounded-2xl border border-white/10 bg-white/4 px-4 py-3 text-white outline-none transition placeholder:text-slate-500 focus:border-amber-400/35"
                    ...
                    <label class="block">
                        <span class="mb-2 block text-sm font-medium text-slate-200">{{ __('site.auth.passwords.reset.password_confirmation') }}</span>
                        <input
                            type="password"
                    ...
                    <div class="flex flex-wrap gap-4">
                        <button type="submit" class="primary-cta rounded-full px-8 py-4 text-base font-semibold">
                            {{ __('site.auth.passwords.reset.submit') }}
                        </button>
