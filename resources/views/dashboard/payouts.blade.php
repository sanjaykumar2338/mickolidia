    ...
    <div class="space-y-6">
        ...
        <div class="grid gap-6 xl:grid-cols-[1fr_0.9fr]">
            <section class="surface-panel rounded-[2rem] p-6">
                ...
                <p class="mt-3 max-w-3xl text-sm leading-7 text-slate-400">{{ __('Payout timing now reads from the linked trading account lifecycle, funded state, and stored payout eligibility fields.') }}</p>
                ...
                <div class="mt-6 space-y-3">
                    ...
                    <div class="rounded-2xl border border-white/8 bg-white/3 px-4 py-4 text-sm text-slate-300">
                        {{ __('Current payout status: :status', ['status' => $payoutSummary['status']]) }}
                    </div>
                    <div class="rounded-2xl border border-white/8 bg-white/3 px-4 py-4 text-sm text-slate-300">
                        {{ __('Eligible profit follows the stored profit split and only becomes available once the account reaches funded payout conditions.') }}
                    </div>
                </div>
            </section>
