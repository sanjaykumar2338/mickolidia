<div data-fixed-disclaimer class="pointer-events-none fixed inset-x-3 bottom-3 z-40 md:bottom-5">
    <div class="pointer-events-auto mx-auto flex max-w-6xl flex-col gap-3 rounded-[1.6rem] border border-white/8 bg-slate-950/92 px-4 py-3 shadow-2xl shadow-black/35 backdrop-blur-xl md:flex-row md:items-center md:justify-between md:px-5">
        <div class="min-w-0">
            <p class="text-[10px] font-semibold uppercase tracking-[0.22em] text-slate-500">{{ __('site.fixed_disclaimer.label') }}</p>
            <p class="mt-1 text-[12px] leading-5 text-slate-400 md:text-[13px] md:leading-6">{{ __('site.fixed_disclaimer.text') }}</p>
        </div>

        <div class="flex shrink-0 flex-wrap items-center gap-2 md:flex-nowrap">
            <a href="{{ route('faq') }}" class="rounded-full border border-white/10 px-4 py-2 text-[11px] font-semibold uppercase tracking-[0.16em] text-slate-200 transition hover:border-white/20 hover:bg-white/6 hover:text-white">
                {{ __('site.fixed_disclaimer.faq_link') }}
            </a>
            <a href="{{ route('payout-policy') }}" class="rounded-full border border-white/10 bg-white/4 px-4 py-2 text-[11px] font-semibold uppercase tracking-[0.16em] text-slate-200 transition hover:border-white/20 hover:bg-white/8 hover:text-white">
