...
@section('title', ($setupCopy['title'] ?? __('site.trial.setup.title')).' | '.__('site.meta.brand'))

@section('content')
    <section class="px-6 pt-12 lg:px-8">
        <div class="mx-auto max-w-6xl">
            <span class="section-label">{{ $setupCopy['eyebrow'] ?? __('site.trial.eyebrow') }}</span>
            <div class="mt-5 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                <div>
                    <h1 class="text-4xl font-semibold text-white sm:text-5xl">{{ $setupCopy['title'] ?? __('site.trial.setup.title') }}</h1>
                    <p class="mt-4 max-w-3xl text-base leading-8 text-slate-300">{{ $setupCopy['description'] ?? __('site.trial.setup.description') }}</p>
                </div>
            ...
            <div class="mt-10 grid gap-6 lg:grid-cols-[0.85fr_1.15fr]">
                <aside class="surface-panel rounded-[2rem] p-6">
                    ...
                    <div class="mt-6 space-y-4">
                        @foreach (($setupCopy['steps'] ?? trans('site.trial.setup.steps')) as $index => $step)
                ...
                <div class="space-y-6">
                    <section class="surface-card rounded-[2rem] p-6 sm:p-8">
                        <p class="text-xs font-semibold uppercase tracking-[0.24em] text-sky-200">{{ $setupCopy['pre_connector_label'] ?? __('site.trial.setup.step_two_label') }}</p>
                        <h2 class="mt-3 text-2xl font-semibold text-white">{{ $setupCopy['pre_connector_title'] ?? __('site.trial.setup.open_demo_title') }}</h2>
                        <p class="mt-4 text-sm leading-7 text-slate-300">{{ $setupCopy['pre_connector_copy'] ?? __('site.trial.setup.open_demo_copy') }}</p>
                        <ul class="mt-5 space-y-3 text-sm text-slate-300">
                            @foreach (($setupCopy['important_items'] ?? trans('site.trial.setup.important_items')) as $item)
                                <li class="rounded-2xl border border-white/6 bg-black/15 px-4 py-3">{{ $item }}</li>
                            @endforeach
                        </ul>
                        <a href="{{ $setupCopy['pre_connector_button_url'] ?? $demoRegistrationUrl }}" @if (($setupCopy['show_demo_section'] ?? true) === true) target="_blank" rel="noopener" @endif class="primary-cta mt-6 inline-flex rounded-full px-8 py-4 text-base font-semibold">
                            {{ $setupCopy['pre_connector_button_label'] ?? __('site.trial.setup.open_demo_button') }}
                        </a>
                    </section>

                    @include('trial.partials.mt5-connector', ['connector' => $connector, 'compact' => false, 'connectorCopy' => $connectorCopy])
                        ...
                        @if (($setupCopy['show_demo_section'] ?? true) === true)
                            <form method="POST" action="{{ route('trial.confirm-demo') }}" class="mt-6">
                                ...
                                <button type="submit" class="primary-cta w-full justify-center rounded-full px-8 py-4 text-base font-semibold">
                                    {{ $setupCopy['continue_button'] ?? __('site.trial.setup.continue_button') }}
                                </button>
                            </form>
                        @else
                            <a href="{{ $setupCopy['continue_url'] ?? route('dashboard') }}" class="primary-cta mt-6 inline-flex w-full justify-center rounded-full px-8 py-4 text-base font-semibold">
                                {{ $setupCopy['continue_button'] ?? __('Open Dashboard') }}
                            </a>
                        @endif
                    </section>

                    <section class="rounded-[2rem] border border-sky-400/18 bg-sky-500/10 p-6 text-sm leading-7 text-sky-50">
                        <p class="font-semibold">{{ $setupCopy['help_title'] ?? __('site.trial.setup.help_title') }}</p>
                        <p class="mt-2">
                            {{ $setupCopy['help_copy'] ?? __('site.trial.setup.help_copy') }}
