...
class TestTelegramWebhook extends Command
{
    ...
    public function handle(): int
    {
        ...
        $token = (string) config('services.metaapi.events.telegram_bot_token', '');
        ...
        if ($send && ! $dryRun && $enabled && $configured) {
            ...
            $result['message'] = $response->successful()
                ? 'Telegram webhook test sent.'
                : 'Telegram webhook returned a non-success status.';
        }

        $this->info('Telegram webhook readiness');
